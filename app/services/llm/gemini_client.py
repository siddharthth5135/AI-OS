...
from app.core.observability.metrics import LLM_ERRORS, LLM_LATENCY, LLM_TOKENS
...
class GeminiClient:
        ...
        try:
            ...
            if os.getenv("TESTING_MOCK_LLM") == "1":
                ...
                # 2. Memory preferences checks
                    ...
                    "user prefers python" in prompt_lower
                    or "python preference" in prompt_lower
                ):
                    text = "Factual memory updated: User prefers Python."
                elif (
                    "my prefs" in prompt_lower
                    or "my preferences" in prompt_lower
                    or "programming preferences" in prompt_lower
                ):
                ...
                # 3. Contextual code generation from memory
                    ...
                    "show me code" in prompt_lower
                    or "show me some code" in prompt_lower
                    or "write async file reader" in prompt_lower
                ):
                ...
                elif "typescript" in prompt_lower or (
                    system_prompt and "typescript" in system_prompt.lower()
                ):
                    text = (
                        "```typescript\n"
                        "interface User {\n"
                ...
                try:
                    response = await asyncio.wait_for(
                        asyncio.to_thread(_sync_generate), timeout=30.0
                    )
                    ...
                    raise google.api_core.exceptions.GatewayTimeout(
                        "LLM generate call timed out."
                    )

            # Record metrics
            latency_seconds = time.time() - start_time
            LLM_LATENCY.labels(agent="general", model=settings.gemini_model).observe(
                latency_seconds
            )
            LLM_TOKENS.labels(agent="general", direction="prompt").inc(
                response_obj.prompt_tokens
            )
            LLM_TOKENS.labels(agent="general", direction="completion").inc(
    ...
    async def generate_stream(
        ...
        try:
            ...
            if os.getenv("TESTING_MOCK_LLM") == "1":
                ...
                try:
                    response = await asyncio.wait_for(
                        asyncio.to_thread(_sync_generate_stream), timeout=30.0
                    )
                except asyncio.TimeoutError:
                    logger.error("llm.stream_init_timeout", timeout=30.0)
                    raise google.api_core.exceptions.GatewayTimeout(
                        "LLM stream call timed out."
                    )
    ...
    async def generate_with_history(self, messages: List[ChatMessage]) -> LLMResponse:
        ...
        try:
            response = await asyncio.wait_for(
                asyncio.to_thread(_sync_send), timeout=30.0
            )
            latency_ms = int((time.time() - start_time) * 1000)
    ...
    async def count_tokens(self, text: str) -> int:
        ...
        try:
            response = await asyncio.wait_for(
                asyncio.to_thread(_sync_count), timeout=10.0
            )
            return response.total_tokens
        except asyncio.TimeoutError:
            logger.error("llm.count_tokens_timeout", timeout=10.0)
            raise google.api_core.exceptions.GatewayTimeout(
                "LLM count tokens call timed out."
            )
