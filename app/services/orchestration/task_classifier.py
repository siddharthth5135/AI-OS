...
class TaskClassification(BaseModel):
    primary_agent: TaskType = Field(..., description="The main agent designed for this task.")
    secondary_agents: List[TaskType] = Field(default_factory=list, description="Other agents that could support this task.")
    ...
    requires_memory: bool = Field(default=False, description="Whether memory retrieval is required.")
    requires_documents: bool = Field(default=False, description="Whether document context retrieval is required.")
    estimated_complexity: str = Field(default="medium", description="Estimated task complexity (low, medium, high).")

class TaskClassifier:
    CODE_SIGNALS = [
        "def ", "class ", "```", "import ", "function", "debug", "error", "bug", "fix",
        "syntax", "python", "javascript", "implement", "algorithm", "traceback",
        "write", "async", "code", "reader", "program", "develop", "show me code"
    ]
    DOCUMENT_SIGNALS = [
        "document", "pdf", "file", "upload", "what does the file", "from the doc", "summarize"
    ]
    MEMORY_SIGNALS = [
        "remember", "last time", "previous", "we discussed", "earlier", "what did i",
        "prefs", "preferences", "know about me", "about my"
    ]
    WORKFLOW_SIGNALS = [
        r"first.*then", "step by step", "and after", "workflow", "multiple steps"
    ]

    async def classify(self, query: str, context: Optional[dict] = None) -> TaskClassification:
        ...
        if context and context.get("doc_ids"):
            logger.info("llm.classify_document_override", doc_count=len(context["doc_ids"]))
        ...
        if confidence >= 0.5:
            logger.info("llm.classify_heuristic_success", primary=primary.value, confidence=confidence)
            classification = TaskClassification(
                primary_agent=primary,
                secondary_agents=[k for k, v in scores.items() if v > 0.2 and k != primary],
                ...
                estimated_complexity="high" if scores[TaskType.CODE] > 0.5 or scores[TaskType.WORKFLOW] > 0.4 else "medium"
            )
        else:
        ...
        if classification.confidence < 0.3:
            logger.info("llm.classify_low_confidence_fallback", confidence=classification.confidence)
