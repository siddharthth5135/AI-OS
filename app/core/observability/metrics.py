...
# HTTP
HTTP_REQUESTS = Counter("ai_os_http_requests_total",
  "Total requests", ["method","endpoint","status"])
HTTP_LATENCY = Histogram("ai_os_http_latency_seconds",
  "Request latency", ["method","endpoint"],
  buckets=[.01,.025,.05,.1,.25,.5,1,2.5,5,10])

# LLM
LLM_TOKENS = Counter("ai_os_llm_tokens_total",
  "LLM tokens used", ["agent","direction"])  # direction: prompt|completion|total
LLM_LATENCY = Histogram("ai_os_llm_latency_seconds",
  "LLM call latency", ["agent","model"],
  buckets=[.5,1,2,3,5,10,20,30,60])
...
TASK_LATENCY = Histogram("ai_os_task_latency_seconds",
  "Task duration", ["type","agent"],
...
# Documents & Memory
DOCS_PROCESSED = Counter("ai_os_docs_processed_total", "Docs indexed", ["file_type","status"])
