...
async def health_check():
    ...
    results = {
        "postgres": checks[0] if not isinstance(checks[0], Exception) else {"ok": False, "error": str(checks[0])},
        "redis":    checks[1] if not isinstance(checks[1], Exception) else {"ok": False, "error": str(checks[1])},
        "pgvector": checks[2] if not isinstance(checks[2], Exception) else {"ok": False, "error": str(checks[2])},
        "gemini":   checks[3] if not isinstance(checks[3], Exception) else {"ok": False, "error": str(checks[3])},
    }
...
async def _check_supabase_storage() -> dict:
    try:
        ...
        async with httpx.AsyncClient() as client:
            res = await asyncio.wait_for(client.get(f"{qdrant_url}/collections", timeout=2.0), timeout=2.0)
...
async def prometheus_metrics(request: Request):
    try:
        ...
        allowed, remaining = await CacheService.rate_limit_check(f"metrics:{client_ip}", limit=10, window_seconds=60)
        if not allowed:
            raise HTTPException(
                status_code=429,
...
@router.get("/api/v1/admin/stats", dependencies=[Depends(require_admin)])
async def admin_stats(db: AsyncSession = Depends(get_db)):
    ...
    # 3. Tasks by status
    res_status = await db.execute(select(Task.status, func.count(Task.id)).group_by(Task.status))
