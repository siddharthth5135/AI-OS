...
async def websocket_chat(
    ...
    try:
        await websocket.send_json({"type": "connected", "data": {"session_id": session_id, "user_id": str(user.id)}})
    ...
    try:
        while True:
            try:
                ...
                try:
                    await websocket.send_json({"type": "error", "data": {"message": f"Invalid message format: {str(e)}", "code": "invalid_format"}})
                    WS_MESSAGES.labels(direction="sent").inc()
                    continue
                except Exception:
                    break
            ...
            if data.get("type") == "chat":
                ...
                if not query:
                    continue
                await handle_chat_ws(websocket, user, session_id, query, data.get("doc_ids", []), db)
...
async def handle_chat_ws(ws: WebSocket, user, session_id: str, query: str, doc_ids: list, db: AsyncSession):
    ...
    try:
        await ws.send_json({"type": "task_update", "data": {"status": "classifying", "task_id": str(task.id)}})
        WS_MESSAGES.labels(direction="sent").inc()
    except Exception:
    ...
    try:
        ...
        try:
            await ws.send_json({"type": "error", "data": {"message": "Processing failed", "code": "internal_error"}})
            WS_MESSAGES.labels(direction="sent").inc()
        except Exception:
            pass
