...
async def test_score_importance():
    ...
    long_text = "This is a very long text designed to exceed two hundred characters in length. " \
...
async def test_short_term_memory(monkeypatch):
    ...
    # Patch get_redis
    monkeypatch.setattr("app.services.memory.memory_service.get_redis", lambda: mock_redis)
    ...
    for i in range(25):
        await service.append_to_session("user123", "session456", {"role": "user", "content": f"msg {i}"})
    res = await service.get_short_term("user123", "session456")
